...
all$Mapper <- factor(all$Mapper,levels = c("BSSeeker2-bwt2-e2e","Bismark-his2","BSSeeker2-bwt","Batmeth2",
                                           "BSSeeker2-bwt2-local","BSSeeker2-soap2","Walt","Bismark-bwt2-e2e","Abismal",
                                           "BSMAP","BSBolt","Hisat_3n","Hisat_3n_repeat","Bwa-meth"))
all$Species <- factor(all$Species,levels = c("Human","Cattle","Pig"))
################# Figure 2c (right): runtime ###########

tiff("figure\\realCputime.tiff",width = 4000,height =3700,res = 600)
ggplot(all,aes(x=Mapper,y=cpuTimeMean,group=Species))+
  geom_line(aes(colour=Species)) +
  #  geom_hline(yintercept = c(100,200,300), color = 'gray') +
...
tiff("figure\\realRss.tiff",width = 4000,height =3700,res = 600)
ggplot(all,aes(x=Mapper,y=RSSMean,group=Species))+
  ...
  theme(axis.title.y = element_text( size = 17,colour ="black",face = "bold"),
        ...
        panel.grid.major =element_blank(),panel.grid.minor = element_blank(),panel.background = element_blank())
dev.off()
################# Figure 3c: uniquely mapped reads #########################
tiff("figure\\realUniqueMapRate.tiff",width = 4000,height =3700,res = 600)
