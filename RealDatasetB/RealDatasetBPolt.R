...
library(stringr)
library(VennDiagram)
############## Figure 5a: CpG: The total number of CpG sites ################################# 
speRes<- function(species,species2,sampleName1,sampleName2,sampleName3,sampleName4,sampleName5,sampleName6,depth){
  ...
  res<- data.frame(Value=c(me(sampleReport1,sampleReport2,sampleReport3,sampleReport4,sampleReport5,sampleReport6)),
                   Mapper=c("Bismark-bwt2-e2e","BSMAP","Bwa-meth","Walt","BSBolt"),
                   Species=species2)
  return(res)
}
human10 <- speRes("human","Human","SRR6373932","SRR6818517","SRR6373926","SRR6373923","SRR6825466","SRR6825471","10")
cattle10 <- speRes("cattle","Cattle","SRR7528450","SRR7528456","SRR7528458","SRR7528459","SRR7528464","SRR7528465","10")
pig10 <- speRes("pig","Pig","SRR7812176","SRR7812178","SRR7812179","SRR7812199","SRR7812200","SRR7812210","10")
allRes <- rbind(human10,cattle10,pig10)
allRes$Mapper <- factor(allRes$Mapper,levels = c("Bwa-meth","BSBolt","BSMAP","Bismark-bwt2-e2e","Walt"))
allRes$Species <- factor(allRes$Species,levels = c("Human","Cattle","Pig"))
tiff("figure/cpg/numOfCpGMapper.tiff",width = 3300,height =2540,res = 600)
ggplot(allRes, aes(Mapper, weight = Value)) +
  ...
  theme(axis.text.x = element_text(angle = 42, hjust = 1,colour ="black",face = "bold",size = 13),
...
############## Figure 5b: The number of shared CpG sites ##################################################
speRes<- function(species,species2,sampleName1,sampleName2,sampleName3,sampleName4,sampleName5,sampleName6,depth){
  sampleReport1 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName1,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport2 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName2,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport3 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName3,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  ...
  res<- data.frame(Value=c(me(sampleReport1,sampleReport2,sampleReport3,sampleReport4,sampleReport5,sampleReport6)),
                   Mapper=c("Bwa,BSM,Bis,Wal","Bwa,BSM,Bis,BSB","BSM,Bis,Wal,BSB","Bwa,Bis,Wal,BSB",
                            "Bwa,BSM,Wal,BSB","Bwa,BSM,Bis,Wal,BSB"),
                   Species=species2)
  return(res)
}
...
pig10 <- speRes("pig","Pig","SRR7812176","SRR7812178","SRR7812179","SRR7812199","SRR7812200","SRR7812210","10")
allRes <- rbind(human10,cattle10,pig10)
allRes$Mapper <- factor(allRes$Mapper,levels = c("Bwa,BSM,Bis,Wal,BSB","Bwa,Bis,Wal,BSB","Bwa,BSM,Wal,BSB",
                                                 "BSM,Bis,Wal,BSB","Bwa,BSM,Bis,BSB","Bwa,BSM,Bis,Wal"))
allRes$Species <- factor(allRes$Species,levels = c("Human","Cattle","Pig"))

tiff("figure/cpg/concordantCpG.tiff",width = 4100,height =3040,res = 600)
...
############## Figure 5c and Figure S8b: CpG：the numnber of reliable CpG sites #################################################################
speRes<- function(species,species2,sampleName1,sampleName2,sampleName3,sampleName4,sampleName5,sampleName6,depth){
  sampleReport1 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName1,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport2 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName2,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport3 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName3,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  ...
  res<- data.frame(Value=c(me(sampleReport1,sampleReport2,sampleReport3,sampleReport4,sampleReport5,sampleReport6)),
                   Mapper=c("Bwa,BSM,Bis,Wal","Bwa,BSM,Bis,BSB","BSM,Bis,Wal,BSB","Bwa,Bis,Wal,BSB",
                            "Bwa,BSM,Wal,BSB","Bwa,BSM,Bis,Wal,BSB"),
                   Species=species2)
  res2<- data.frame(value=c(sum(res[c(1,2,4,5),1])-3*res[6,1], ## Bwa-meth 
                            sum(res[c(2,3,4,5),1])-3*res[6,1], ## BSBolt 
                            sum(res[c(1,2,3,5),1])-3*res[6,1], ## BSMAP
                            sum(res[c(1,2,3,4),1])-3*res[6,1], ## Bismark-bwt2-e2e
                            sum(res[c(1,3,4,5),1])-3*res[6,1]), ## Walt
                    
                    Rate=c((sum(res[c(1,2,4,5),1])-3*res[6,1])/(sum(res[c(1,2,3,4,5),1])-4*res[6,1]), ## Bwa-meth 
                           (sum(res[c(2,3,4,5),1])-3*res[6,1])/(sum(res[c(1,2,3,4,5),1])-4*res[6,1]), ## BSBolt 
                           (sum(res[c(1,2,3,5),1])-3*res[6,1])/(sum(res[c(1,2,3,4,5),1])-4*res[6,1]), ## BSMAP 
                           (sum(res[c(1,2,3,4),1])-3*res[6,1])/(sum(res[c(1,2,3,4,5),1])-4*res[6,1]), ## Bismark-bwt2-e2e
                           (sum(res[c(1,3,4,5),1])-3*res[6,1])/(sum(res[c(1,2,3,4,5),1])-4*res[6,1])), ## Walt
                    
                    mapper=c("Bwa-meth","BSBolt","BSMAP","Bismark-bwt2-e2e","Walt"),
                    Class=c("Covered by four alignment algorithms in Bwa-meth",
                            "Covered by four alignment algorithms in BSBolt",
                            "Covered by four alignment algorithms in BSMAP",
                            "Covered by four alignment algorithms in Bismark-bwt2-e2e",
                            "Covered by four alignment algorithms in Walt"),
                    Species=species2,
                    SpeCla=c(paste0(species,": Covered by three alignment algorithms in Bwa-meth"),
                             paste0(species,": Covered by three alignment algorithms in BSBolt"),
...
allRes$Class <- factor(allRes$Class,levels = c(c("Covered by four alignment algorithms in Bwa-meth",
                                                 "Covered by four alignment algorithms in BSBolt",
                                                 "Covered by four alignment algorithms in BSMAP",
                                                 "Covered by four alignment algorithms in Bismark-bwt2-e2e",
                                                 "Covered by four alignment algorithms in Walt")))
allRes$Species <- factor(allRes$Species,levels = c("Human","Cattle","Pig"))
tiff("figure/cpg/reliable_CpG_rate.tiff",width = 4200,height =3400,res = 600)
...
tiff("figure/cpg/reliable_CpG_value.tiff",width = 4400,height =3400,res = 600)
ggplot(allRes,aes(x=mapper,y=value,group=Species))+
  labs( x="",y = 'The number of accurate CpG sites',shape='Algorithms')+
  geom_point(aes(shape=mapper,colour=Species),size =5,stroke = 2) +
  scale_shape_manual(values = c(1,2,3,4,5))+
  scale_y_continuous(expand = c(0,0),limits=c(4.0e+06,1.6e+07),breaks=seq(4.0e+06,1.6e+07,3.0e+06)) +
  theme(axis.title.y = element_text( size = 15,colour ="black",face = "bold"),
        axis.text.y = element_text( size = 15,colour ="black",face = "bold"),
        axis.text.x = element_text(angle = 47, hjust = 1, size = 15,colour ="black",face = "bold"),
        axis.line = element_line(color="black"),
...
############## Figure 5d: CpG: The number of shared CpG sites ######################################
...
############## Figure 5e and Figure S9b: CpG：The number of reliable CpG sites #########################
countToalNumCpG <- function(species,sampleName1,sampleName2,sampleName3,sampleName4,sampleName5,sampleName6,depth){
  sampleReport1 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName1,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport2 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName2,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport3 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName3,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
...
############## Figure 5f: CpG: The proportion of discordant and concordant CpG sites ####################################################
...
print(p)
dev.off()
############## Figure 5g: CpG: The proportion of discordant and concordant CpG sites in cgi and repeat ###########################################################
...
ggplot(allSpecies10, aes(SpeRegion, weight = Rate*100, fill = Type)) +
  ...
  theme(axis.text.y = element_text(colour =c(rep("lightskyblue4",6),rep("palevioletred4",6)),size = 11,face = "bold"),
        ...
        axis.text.x = element_text(size = 12,colour ="black",face = "bold"))
dev.off()
############## Figure 5h: CpG: The proportion of discordant and concordant CpG sites in different methylation levels#######################
...
dev.off()
############## Figure S8a: CpG: Venn for five alignment algorithms ####################################################
speVennCpG<- function(species,sampleName1,sampleName2,sampleName3,sampleName4,sampleName5,sampleName6,depth){
  sampleReport1 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName1,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport2 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName2,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport3 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName3,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport4 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName4,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport5 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName5,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport6 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName6,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  me<- function(report1,report2,report3,report4,report5,report6){
    res <- c()
    for(i in 6:36){
      temp<- mean(c(as.numeric(as.character(report1[i,2])),
                    as.numeric(as.character(report2[i,2])),
                    as.numeric(as.character(report3[i,2])),
                    as.numeric(as.character(report4[i,2])),
                    as.numeric(as.character(report5[i,2])),
                    as.numeric(as.character(report6[i,2]))))
  ...
  classStr <- function(report1){
    res <- c()
    for(i in 6:36){
      temp <- str_sub(as.character(report1[i,1]),end = -4)
      res <- c(res,temp)
    }
    return(res)
  }
  ...
  tiff(paste("figure/cpg/",species,"_",depth,"_DiscordantCpG.tiff",sep = ""),width = 4850,height =4800,res = 600)
  p <- draw.quintuple.venn(
    area1=value("a"), area2=value("b"), area3=value("c"), area4=value("d"), area5=value("e"),
    n12=value("ab"), n13=value("ac"), n14=value("ad"), n15=value("ae"), n23=value("bc"), n24=value("bd"), n25=value("be"), n34=value("cd"), n35=value("ce"), n45=value("de"), 
    n123=value("abc"), n124=value("abd"), n125=value("abe") , n234=value("bcd"), n235=value("bce"), n245=value("bde"), n134=value("acd"),n135=value("ace"),n145=value("ade"), 
    n345=value("cde"),
    n1234=value("abcd"),n1235=value("abce") ,n1245=value("abde"), n1345=value("acde"), n2345=value("bcde"),
    n12345=value("abcde"), category = c('Bismark-bwt-e2e','BSMAP','Bwa-meth','Walt','BSBolt') ,
    col=c("paleturquoise3","pink3","darkseagreen3","khaki3","lightskyblue3"), fill=c("paleturquoise3","pink3","darkseagreen3","khaki3","lightskyblue3"),
    alpha = 0.7 ,
    cex=1.4,
    cat.cex=1.4,
    fontface = "bold",
...
speVennCpG("pig","SRR7812176","SRR7812178","SRR7812179","SRR7812199","SRR7812200","SRR7812210","10")
speVennCpG("human","SRR6373932","SRR6818517","SRR6373926","SRR6373923","SRR6825466","SRR6825471","10")
speVennCpG("cattle","SRR7528450","SRR7528456","SRR7528458","SRR7528459","SRR7528464","SRR7528465","10")

############## Figure S9a: CpG: venn for four alignment algorithms ############################################################################
countToalNumCpG <- function(species,sampleName1,sampleName2,sampleName3,sampleName4,sampleName5,sampleName6,depth){
  sampleReport1 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName1,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport2 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName2,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleReport3 <- read.csv(paste(species,"\\cpgAnaRes\\dpeth",depth,"_",sampleName3,"_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
...
############## Figure 6a: DMC: The total number of DMC  #######################################################
...
ggplot(allRes, aes(Mapper, weight = Number)) +
  ...
  facet_wrap( .~ Species,ncol = 4)
dev.off()
############## Figure 6b: DMC: The number of shared DMC ######################
...
############## Figure 6c and Figure S10b: DMC：The number of reliable DMCs ##############################################
...
ggplot(allRes,aes(x=mapper,y=value,group=Species))+
  ...
  facet_wrap( .~ Species,ncol = 3)
dev.off()
############## Figure 6d: DMC: The proportion of discordant and concordant DMC in cgi and repeat ###############
...
############## Figure S10a: DMC: venn for four alignment algorithms ###############################################################
...
############## Figure 6e: DMR: The total length of DMR ################################
...
ggplot(allRes, aes(Mapper, weight = Number)) +
  ...
  facet_wrap( .~ Species,ncol = 4)
dev.off()
############## Figure 6f: DMR: The length of shared DMR ######################################
DMRres<- function(species,species2,depth){
  sampleReport <- read.csv(paste(species,"\\dmrRes\\depth",depth,"\\",species,"_vennRes.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleRes<- sampleReport[15:19,]
  sampleRes$Type <- c("Bwa,BSM,Bis","BSM,Bis,Wal","Bwa,Bis,Wal","Bwa,BSM,Wal","Bwa,BSM,Bis,Wal")
...
############## Figure 6h: DMR: The proportion of discordant and concordant DMR in cgi and repeat #################################################
...
############## Figure 6g and Figrue S10d: DMR：The number of reliable DMRs #############################################
DMRres<- function(species,species2,depth){
...
ggplot(allRes,aes(x=mapper,y=value,group=Species))+
  ...
  facet_wrap( .~ Species,ncol = 3)
dev.off()
############## Figure S10c: DMR: Venn for four alignment algorithms ##########################################
...
############## Figure 8a: DMR-related genes: The total number of genes ############################
...
ggplot(allRes, aes(Mapper, weight = Number)) +
  ...
  facet_wrap( .~ Species,ncol = 4)
dev.off()
############## Figure 8b: DMR-related genes: The number of shared gene ##########################################################
GENEres<- function(species,species2,depth){
  sampleReport <- read.csv(paste(species,"\\gene\\depth",depth,"\\","depth",depth,"_gene_DMR_Venn_report.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleRes<- sampleReport[15:19,]
  sampleRes$Type <- c("Bwa,BSM,Bis","BSM,Bis,Wal","Bwa,Bis,Wal","Bwa,BSM,Wal","Bwa,BSM,Bis,Wal")
...
############## Figure 8c and Figure S11b: DMR-related genes：The numnber of reliable genes #################################################
GENEres<- function(species,species2,depth){
...
ggplot(allRes,aes(x=mapper,y=value,group=Species))+
  ...
  facet_wrap( .~ Species,ncol = 3)
dev.off()
############## Figure S11a: DMR-related genes: venn for four alignment algorithms ###############################################################
...
############## Figure 8d: KEGG: The total number of KEGG ##############################################
...
ggplot(allRes, aes(Mapper, weight = Number)) +
  ...
  facet_wrap( .~ Species,ncol = 3)
dev.off()
############## Figure 8e: KEGG: The number of shared pathway ######################################################
KEGGres<- function(species,species2,depth){
  sampleReport <- read.csv(paste("enrichKEGG\\depth",depth,"_p001\\",species,"_kegg_Venn.txt",sep=""),header = F,sep = "\t", quote = "\"", dec = ".")
  sampleRes<- sampleReport[15:19,]
  sampleRes$Type <- c("Bwa,BSM,Bis","BSM,Bis,Wal","Bwa,Bis,Wal","Bwa,BSM,Wal","Bwa,BSM,Bis,Wal")
...
############## Figure 8f and Figure S11d: KEGG：The number of reliable pathway ############################################
KEGGres<- function(species,species2,depth){
...
############## Figure S11c: KEGG: venn for for alignment algorithms #############################################################
...
keggVenn("cattle","10")
keggVenn("pig","10")
############## Figure S12: KEGG: top 30 pathway ################################
