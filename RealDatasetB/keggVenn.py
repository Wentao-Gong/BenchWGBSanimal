...
import argparse
import os
parser = argparse.ArgumentParser(description='The venn results of KEGG')
...
def geneVenn(aFile,bFile,cFile,dFile,outputFile):
    ...
    aMapperName=os.path.basename(aFile).split('_')[1]
    bMapperName=os.path.basename(bFile).split('_')[1]
    cMapperName=os.path.basename(cFile).split('_')[1]
    dMapperName=os.path.basename(dFile).split('_')[1]
    aSet=set()
    bSet=set()
    cSet=set()
    dSet=set()
